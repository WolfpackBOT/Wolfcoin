...
const char * const BITCOIN_CONF_FILENAME = "wolfcoin.conf";
const char * const BITCOIN_PID_FILENAME = "wolfcoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "wolfcoin" is a composite category enabling all Wolfcoin-related debug output
                if(ptrCategory->count(std::string("wolfcoin"))) {
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Wolfcoin";
#else
    // Unix
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty wolfcoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override wolfcoin.conf
