...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // ignore all Wolfcoin related functionality
...
/*
    ...
    allow endless transaction that would bloat Wolfcoin and make it unusable. To
...
{
    if(fLiteMode) return; // disable all Wolfcoin specific functionality
    ...
    RenameThread("wolfcoin-ps-server");
